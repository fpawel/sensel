return {
    Samples = {
        ...
        sample(4, "1m30s", 0.160, 40)
    },
    Calculate = function (U, I, T, C)

        -- Нижний концентрационный предел распространения пламени H2, [% объёмной доли]
        local LEL = 4.0
        ...
        -- Действительное напряжение на ЧЭ на ГС4 и токе I3 в ходе обмера [В]
        local Ugs = U[3]
        
        -- Чувствительность ЧЭ в схеме моста Уинстона [мВ/НКПР]
        local K = 1000 * (LEL / (100 * C[4])) * (Ugs - Ur) / 2

        return {
            {
                Name = "R0",
                Value = R0,
                Ok = R0 >= 3.65 and R0 <= 3.95,
                Precision = 3,
            },
            {
                Name = "Uизм",
            ...
            },
            {
                Name = "B",
                Value = B,
                Ok = B >= 15000 and B <= 18000,
                Precision = 0,
            },
            {
                Name = "U20",
                Value = U20,
                Ok = U20 >= 1.520 and U20 <= 1.750,
                Precision = 3,
            },
            {
                Name = "K",
